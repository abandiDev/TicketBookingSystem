...
public class ShowTimeService : IShowTimeService
{
    ...
    {
        var reservation = await _dbContext.Reservations.Include(x => x.Showtime)
            .FirstOrDefaultAsync(x => x.Id == reservationId && x.ShowtimeId == showTimeId);
        if (reservation == null)
        {
            ...
            .FromSqlInterpolated(
                $@"SELECT 
                   date_part('hour', ""BookingTime"") AS BookingHour,
                    ...
                    SUM(CASE WHEN ""Status"" = 0 THEN ""NumberOfSeats"" ELSE 0 END) AS TotalSeatsBooked
                FROM ""Reservations""
